...
void bin_search_tree::b_search_tree(node* temp)
{
    ...
    if (temp->info == number)
    {
        cout << " \n Given number is already present in the tree.\n";
        return;
    }
...
void bin_search_tree::in_order(node *temp, int level)
{
    if (root == NULL)
    {
    ...
    if (temp->left != NULL)
        in_order(temp->left, level + 1);

    std::cout << "Node " << temp->info << " is at level " << level << std::endl;

    if (temp->right != NULL)
        in_order(temp->right, level + 1);
